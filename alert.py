...
def get_broad_market_tickers():
    ticker_map = {
        ...
        "010140.KS": "삼성중공업", "010620.KS": "HD현대미포", "011210.KS": "현대위아", "012450.KS": "한화에어로스페이스",
        ...
        "253450.KQ": "스튜디오드래곤", "056190.KQ": "에스에프에이", "036490.KS": "콘텐트리중앙", "025980.KQ": "아난티",
        "032670.KS": "부광약품", "000250.KQ": "삼천당제약", "064550.KQ": "바이오니아", "041190.KQ": "우리기술투자",
        "086900.KQ": "메디포스트", "036200.KQ": "유진테크", "033640.KQ": "네패스", "023590.KQ": "다우기술",
        "053030.KQ": "바이넥스", "042000.KQ": "카페24", "078340.KQ": "컴투스",
        "084110.KQ": "휴온스", "098460.KQ": "고영", "030190.KQ": "나이스정보통신"
    }
    return ticker_map
...
def run_fvg_scanner():
    ...
    print(f"🚀 코스피/코스닥 핵심 종목 총 {len(tickers)}개 전수 조사 시작...")
    ...
    for ticker in tickers:
        try:
            ...
            # 골든크로스 여부 (직전엔 아래, 지금은 위)
            is_macd_gold_cross = (macd.iloc[-2] < signal.iloc[-2]) and (macd.iloc[-1] > signal.iloc[-1])
            
            # SMC 최종 판정
            is_smc_signal = (curr['Close'] > ema60.iloc[-1]) and is_macd_gold_cross
            ...
            # 2. 관망 신호 (Watch)
            is_watch_signal = (is_smc_signal and 75 <= rsi_val < 80) or (30 < rsi_val <= 40)

            # 3. 매도 신호 (Sell)
            is_sell_signal = (rsi_val >= 85) or ("Bearish" in str(fvg_status))

            if is_buy_signal:
                buy_type = "공격적 추세 돌파 🚀" if is_smc_signal else "적극적 눌림목 매수 🎯"
                found_signals.append(
                    f"🔥 *[사야 해!]*: {name}({display_ticker})\n"
                    f"   *현재가*: {int(curr['Close']):,}원\n"
                    f"   *구분*: {buy_type}\n"
                    f"   *RSI*: {rsi_val}\n"
                    f"   *FVG*: {fvg_status if fvg_status else '없음'}"
                )
            elif is_watch_signal:
                watch_type = "고점 돌파 대기 🚦" if rsi_val >= 75 else "바닥 다지기 중 🔍"
                found_signals.append(
